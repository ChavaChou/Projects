...
legalization <- read_excel('legality.xlsx', col_names = TRUE)
# (0 = Fully-illegal, 1 = medical, 2 = Legal)
population <- read_excel('pop.xlsx',col_names = TRUE)
population$State <- str_sub(population$State, start = 2)
age <- read.csv('age_gender.csv', header = TRUE)
# 0 - total, 1 - male, 2 - female
gender <- read.csv('gender.csv', header = TRUE)
politics <- read_excel('politicalparty.xlsx', col_names = TRUE)
# R = 1, D = 2, Neutral= 0
alcohol <- read_excel('alcohol.xlsx', col_names = TRUE)
rev_expenses <- read_excel('rev_expenses.xlsx', col_names = TRUE)
