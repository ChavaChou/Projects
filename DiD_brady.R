...
pop <- pop %>% select(State, Total_Area) %>% rename(state = State)

crashes <- merge(crashes, revenues, by = c('state', 'year'))
crashes <- merge(crashes, pop, by = 'state')
...
# exploratory plots
ggplot(crashes) + geom_line(aes(x = month_num, y = fatal_crashes, color = state)) + geom_vline(xintercept = 25)

crashes %>% group_by(treatment, month_num) %>%
  summarize(f_crashes = mean(fatal_crashes)) %>%
  ggplot() + geom_line(aes(x = month_num, y = f_crashes, color = treatment)) + geom_vline(xintercept = 25)
...
# dropping texas to see what happens
...
# DiD with no weather
# naive regression
summary(lm(fatal_crashes + 1 ~ after + treatment + hw_expense_ratio + after * treatment, data = crashes))

# sfe
did_sfe <- plm(fatal_crashes ~ after + treatment + hw_expense_ratio + after * treatment, data = crashes,
